    ...
    <div className="relative z-0 w-full h-screen flex justify-center items-center">
      ...
      >
        ...
        <Physics gravity={gravity} timeStep={1 / 60}>
          <Band cardGLB={cardGLB} textureURL={lanyardTextureURL} />
        </Physics>
...
function Band({ cardGLB, textureURL, maxSpeed = 50, minSpeed = 0 }) {
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      [j1, j2].forEach((ref) => {
        ...
        ref.current.lerped.lerp(ref.current.translation(), delta * Math.max(minSpeed, Math.min(maxSpeed, dist)));
      });
